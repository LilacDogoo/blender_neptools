...
from blender_neptools import import_to_blender, binary_file_utils
from blender_neptools.matrix4f import Matrix4f

TEXTURE_PATH_DEFAULT = "texture\\001\\"


class ISM2_importOperator(bpy.types.Operator):
...
def read_ism2(filepath: str, filename: str,
              ...
              transform_to_blender_space: Matrix4f = Matrix4f.create_rotation_x(math.pi * .5)) -> blender_neptools.import_to_blender.PreBlender_Model:
    ...
    f.seek(0x14)
    R: binary_file_utils.LD_BinaryReader = binary_file_utils.LD_BinaryReader(f, 0 < binary_file_utils.struct_ULongL.unpack_from(f.read(4))[0] < 65535)

    model = blender_neptools.import_to_blender.PreBlender_Model()
    bboxCount = 0
            ...
            for current_bone_index in range(model.bones.count):  # BONE DATA BLOCK
                ...
                for current_bone_attribute in range(bone_attribute_count):
                    ...
                    if bone_attribute_type == 0x5B:  # Type 91 # Bone Attibute: Transforms (Transforms are relative to parent)
                        ...
                        if blender_neptools.debug: print("      Bone Attribute Type 0x5B == 91 <Transforms>: @ %s" % hex(bone_attribute_offset_array[current_bone_attribute]).rjust(6))
                        ...
                        for current_transform in range(bone_transform_count):
                            ...
                            if bone_transform_type == 0x14:  # 20 # Matrix Translation
                                ...
                                m_trans = (m41, m42, m43)
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Translate(%.4f, %.4f, %.4f)" % (
                            ...
                            #     if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Scale(%.4f, %.4f, %.4f)" % (
                            #         current_transform, hex(current_transform_offset).rjust(6), str(current_transform_offset).ljust(6), hex(bone_transform_type), bone_transform_type, scalex, scaley, scalez))
                                ...
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Matrix.X(%.4f, %.4f, %.4f, %.4f)" % (
                                ...
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Matrix.Y(%.4f, %.4f, %.4f, %.4f)" % (
                                ...
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Matrix.Z(%.4f, %.4f, %.4f,  %.4f)" % (
                            ...
                            # The Vector part seems to be redundant since we already know it base on "bone_transform_type"
                                ...
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Joint.X(%.4f, %.4f, %.4f,  %.4f)" % (
                                ...
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Joint.Y(%.4f, %.4f, %.4f,  %.4f)" % (
                                ...
                                if blender_neptools.debug: print("        Bone Tansform %s: @ %s == %s  Type=%s == %s          Joint.Z(%.4f, %.4f, %.4f,  %.4f)" % (
                        ...
                        for current_surface in range(bone_attribute_surfaces_count):
                            ...
                            if blender_neptools.debug: print("        Surface %s: @ %s  mat=\"%s\"  tex=\"%s\"" % (current_surface, hex(bone_attribute_surface_offset_array[current_surface]),
                                                                                                                   model.strings[surface_name__string_id], model.strings[surface_material_name__string_id]))
                    elif bone_attribute_type == 0x4D:  # Type 92 # Bone Attribute: Shape Keys
                        if blender_neptools.debug: print("      Bone Attribute Type 0x5C == 92  Shape Keys  @ %s" % hex(bone_attribute_offset_array[current_bone_attribute]).rjust(6))

                    elif bone_attribute_type == 0x5C:  # Type 92 # Bone Attribute: Child Bone List
                        if blender_neptools.debug: print("      Bone Attribute Type 0x5C == 92  Child Bone List  @ %s" % hex(bone_attribute_offset_array[current_bone_attribute]).rjust(6))
                    else:
                        if blender_neptools.debug: print("      Bone Attribute Type %s == %s  <not-implemented>  @ %s" % (hex(bone_attribute_type), bone_attribute_type, hex(bone_attribute_offset_array[current_bone_attribute]).rjust(6)))
                model.bones.append(current_bone)
                if blender_neptools.debug: print(current_bone)
...
def parse_face_anm(filepath: str) -> import_to_blender.FaceAnm:  # returns None if parsing fails
    ...
    f = open(filepath, 'rb')
    # Check Endian -- This is a sort of count which will always have a low positive integer
    # Stream Reader Functions :: Some are created based on endian
    f.seek(4)
    R: binary_file_utils.LD_BinaryReader = binary_file_utils.LD_BinaryReader(f, 0 < binary_file_utils.struct_ULongL.unpack_from(f.read(4))[0] < 0x10000)
